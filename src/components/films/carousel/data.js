export const data = [
    {
        'uid' : 'uid1',
        'title' : 'After: En mil pedazos',
        'duration' : '1h 45m',
        'description' : 'Ha pasado el tiempo, y Hardin (Hero Finnes Tiffin) todavía no se sabe si es realmente el chico profundo y reflexivo del que Tessa (Josephine Langford) se enamoró, o ha sido un extraño todo este tiempo. Ella quiere alejarse, pero no es tan fácil. Tessa se ha centrado en sus estudios y comienza a trabajar como becaria en Vance Publishing. Allí conoce a Trevor (Dylan Sprouse), un nuevo y atractivo compañero de trabajo que es exactamente el tipo de persona con la que debería estar. Pero Hardin sabe que cometió un error, posiblemente el más grande de su vida y quiere corregir sus errores y vencer a sus demonios... Secuela de la película "After".',
        'trailer' : 'Qxfqpe3IG7o'
    },
    {
        'uid' : 'uid2',
        'title' : 'Avengers: Endgame / Vengadores 4',
        'duration' : '3h 01m',
        'description' : 'Culminación de un universo de 22 películas interconectadas, la cuarta entrega de la saga Avengers invita al público a presenciar el momento decisivo de este viaje épico. Nuestros queridos héroes comprenderán por fin lo frágil que es esta realidad y los sacrificios que deberán hacer para defenderla.',
        'trailer' : '9hpWz0ZMFAo'
    },
    {
        'uid' : 'uid3',
        'title' : 'El Halloween de Hubie',
        'duration' : '1h 42m',
        'description' : 'Pese a su devoción por el pueblo de Salem, Hubie Dubois (Sandler) reconoce que la noche de Halloween no es precisamente la más atractiva para los jóvenes desde hace unos años. En esta ocasión, no obstante, algo distinto está a punto de suceder, y sólo él podrá salvar esta mágica noche.',
        'trailer' : 'npHt2xDI2r4'
    },
    {
        'uid' : 'uid4',
        'title' : 'Estación zombie 2: Península',
        'duration' : '1h 56m',
        'description' : 'Después de que la infección que se extendió hace 4 años, solo algunas partes de Corea del Sur permanecen resguardadas, aunque la Península de Busan es la única parte en donde pueden buscar refugio los coreanos, muchos aún mantienen la esperanza de ser rescatados por ayuda internacional.',
        'trailer' : 'owmIQ906Lp8'
    },
    {
        'uid' : 'uid5',
        'title' : 'La deuda 2',
        'duration' : '1h 37m',
        'description' : 'French y Sue trabajan en lo que mejor se les da -romper cráneos y aplastar huesos- mientras persiguen a los criminales de poca monta que le deben dinero a Tommy, su jefe. Cuando reciben una citación para ir a buscar un dinero en un casino de Las Vegas, éste resulta ser un antiguo amante de Sue. Mientras tanto, un importante traficante de droga se propone encontrar a la pareja para vengar la muerte de su hermano. Sin otro remedio que luchar para sobrevivir, French y Sue tendrán que abrirse paso ante todo aquel que trate de interponerse en su camino.... Secuela de "La deuda" (The Debt Collector).',
        'trailer' : 'tTlhmJ1yT-0'
    },
    {
        'uid' : 'uid6',
        'title' : 'Los nuevos mutantes',
        'duration' : '1h 34m',
        'description' : 'La historia de los Nuevos Mutantes, un equipo de héroes compuesto por los primeros graduados de la escuela de Charles Xavier.',
        'trailer' : 'Eup_tsKZ5ek'
    },
    {
        'uid' : 'uid7',
        'title' : 'Mulán',
        'duration' : '1h 55m',
        'description' : 'El Emperador de China emite un decreto para reclutar a un varón por cada familia que deberá servir en el Ejército Imperial para defender al país de los invasores del Norte. Hua Mulán, hija única de un condecorado guerrero, se presenta para evitar que su anciano padre sea llamado a filas. Se hace pasar por un hombre, Hua Jun y se somete a todo tipo de pruebas. Para conseguirlo deberá apelar a su fuerza interior y sacar a la luz todo su potencial. Se trata de una aventura épica que la transformará en una guerrera laureada que la hará merecedora del respeto de todo un país y motivo de orgullo para su padre. “Mulán" cuenta con un famoso reparto internacional que incluye a Yifei Liu como Mulán; a Donnie Yen como el Comandante Tung; a Jason Scott Lee como Böri Khan; a Yoson An como Cheng Honghui; con Gong Li como Xianniang y Jet Li como el Emperador.',
        'trailer' : '1dnaQjFL67k'
    },
    {
        'uid' : 'uid8',
        'title' : 'Tenet',
        'duration' : '2h 30m',
        'description' : 'Armado con tan solo una palabra –Tenet– el protagonista de esta historia deberá pelear por la supervivencia del mundo entero en una misión que le lleva a viajar a través del oscuro mundo del espionaje internacional, y cuya experiencia se desdoblará más allá del tiempo lineal.',
        'trailer' : 'AZGcmvrTX9M'
    },
    {
        'uid' : 'uid9',
        'title' : 'The Witches',
        'duration' : '1h 39m',
        'description' : 'Un niño de siete años protagoniza una serie de aventura con un grupo de brujas. Adaptación del libro "Las brujas", aparecido en 1973 y firmado por Roald Dahl.',
        'trailer' : '9nlhmJF5FNI'
    }
]